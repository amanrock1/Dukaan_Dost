...
// WinAnsi (Helvetica) only supports Latin chars (0x00–0xFF).
// Strip any character outside that range so PDF generation never crashes.
function sanitizeText(text: string): string {
  if (!text) return '';
  // Replace common Devanagari/Unicode letters with transliterated ASCII equivalent
  // then strip anything still outside WinAnsi range
  return text
    .replace(/[\u0900-\u097F]/g, (ch) => {
      // Basic Devanagari vowel/consonant to rough Latin mapping
...
export async function generateInvoice(saleId: string): Promise<{ success: boolean; pdfData?: string; message: string }> {
  try {
    ...
    // Table row — sanitize all text before drawing
    y -= 32;
    page.drawText('1', { x: 40, y, size: 10, font, color: darkGray });
    page.drawText(sanitizeText(sale.product.name), { x: 65, y, size: 10, font, color: darkGray });
    ...
    page.drawText(sanitizeText(rateText), { x: 370, y, size: 10, font, color: darkGray });
    const amtText = 'Rs. ' + sale.amount.toLocaleString('en-IN');
    page.drawText(sanitizeText(amtText), { x: 440, y, size: 10, font, color: darkGray });
    ...
    // Amount in words — sanitize too
    y -= 40;
    const wordsText = 'Amount in Words: ' + sanitizeText(numberToWords(sale.totalAmount)) + ' Rupees Only';
    page.drawText(sanitizeText(wordsText), { x: 40, y, size: 9, font: fontBold, color: darkGray });
    ...
    const footerText = sanitizeText('Generated by DukaanDost AI - ' + BUSINESS.gstin);
    ...
    return { success: true, pdfData: pdfBase64, message: 'Invoice ' + invNumber + ' generated for ' + sanitizeText(sale.product.name) + '.' };
